window.__ModuleLoader__.load({ id: "dsh-inline-chat", factory: (require) => {
...
//#endregion
require("@deepseek-ai/dsh-client-connection/client");

//#region src/core.ts
/** dsh-inline-chat 的跨端协议。只包含可序列化的数据，不依赖浏览器或 Node。 */
const INLINE_CHAT_CHANNEL = "/dsh-inline-chat";
const INLINE_CHAT_STORAGE_KEY = "dsh-inline-chat:v1";
const INLINE_CHAT_MODEL_STORAGE_KEY = "dsh-inline-chat:model:v1";
...
const name = "dsh-inline-chat-client";
const inject = [
	"connection",
	"remote",
	"remote.session",
...
function mount(ctx) {
	const old = document.querySelector("[data-dsh-inline-chat-root]");
	const oldDispose = old?.__dshInlineChatDispose;
	oldDispose?.();
	old?.remove();
	...
	const button = mustElement(shadow, "[data-action=\"toggle\"]");
	const panel = mustElement(shadow, "[data-panel]");
	const close = mustElement(shadow, "[data-action=\"close\"]");
	const fresh = mustElement(shadow, "[data-action=\"new\"]");
	const form = mustElement(shadow, "form");
	const input = mustElement(shadow, "textarea");
	const submit = mustElement(shadow, "[data-action=\"send\"]");
	const list = mustElement(shadow, "[data-messages]");
	const status = mustElement(shadow, "[data-status]");
	const modelSelect = mustElement(shadow, "[data-model-select]");
	mustElement(shadow, "[data-effort-wrap]");
	const effortSelect = mustElement(shadow, "[data-effort-select]");
	const effortShell = mustElement(shadow, "[data-effort-wrap]");
	let messages = loadMessages();
	let modelGroups = [];
	let selection = loadModelSelection();
	let selectionSource = "follow";
	...
	const render = () => {
		...
		const openStates = Array.from(list.children).map((el) => {
			return (el.querySelector?.("details"))?.open ?? false;
		});
		list.innerHTML = messages.length ? messages.map((message, index) => renderMessage(message, openStates[index] ?? false)).join("") : emptyStateMarkup();
	...
	let streamingArticle;
	const renderStreaming = () => {
		const last = messages.at(-1);
		if (!last || last.role !== "assistant" || last.error) {
			streamingArticle = void 0;
			render();
			return;
		}
		let article = streamingArticle;
		if (!article || !article.isConnected || list.lastElementChild !== article || list.children.length !== messages.length) {
			render();
			article = list.lastElementChild instanceof HTMLElement ? list.lastElementChild : void 0;
	...
	const cancelActive = () => {
		...
		controller = void 0;
		sending = false;
		if (requestId) ctx.connection.rpc.call(INLINE_CHAT_CHANNEL, "cancel", { requestId });
	};
	const reset = () => {
		cancelActive();
		messages = [];
	...
	const send = async () => {
		...
		const requestSelection = selection ? { ...selection } : void 0;
		...
		render();
		try {
			const result = await ctx.connection.rpc.call(INLINE_CHAT_CHANNEL, "start", {
				messages: messages.filter((message) => message.text.trim()).map(({ role, text: value }) => ({
					role,
					text: value
				})),
				selection: requestSelection
			}, requestController.signal);
			if (!isRpcSuccess(result) || !isRecord(result.value) || typeof result.value.requestId !== "string") throw new Error(isRpcFailure(result) ? result.error.message : "辅助 Chat 启动失败");
			const requestId = result.value.requestId;
			activeRequestId = requestId;
			while (currentGeneration === generation && !requestController.signal.aborted) {
				const poll = await ctx.connection.rpc.call(INLINE_CHAT_CHANNEL, "poll", { requestId }, requestController.signal);
				if (!isRpcSuccess(poll) || !isRecord(poll.value)) throw new Error(isRpcFailure(poll) ? poll.error.message : "辅助 Chat 获取回复失败");
				const value = poll.value;
				if (!Array.isArray(value.events) || typeof value.done !== "boolean") throw new Error("辅助 Chat 返回内容无效");
			...
			if (currentGeneration === generation && !receivedAssistant) throw new Error("模型没有返回可显示的内容");
		} catch (error) {
	...
	const appendAssistant = (text) => {
		const last = messages.at(-1);
		if (!last || last.role !== "assistant" || last.error) messages.push({
			role: "assistant",
			text: ""
		});
	...
	const setEffortVisible = (visible) => {
		effortShell.hidden = !visible;
		effortShell.dataset.visible = String(visible);
	};
	const renderEffortOptions = () => {
		effortSelect.replaceChildren();
		const efforts = ((selection ? findModel(modelGroups, selection) : void 0)?.reasoning?.efforts ?? []).filter((effort) => typeof effort?.id === "string" && effort.id.trim().length > 0 && typeof effort?.name === "string" && effort.name.trim().length > 0);
		if (!efforts.length) {
			setEffortVisible(false);
	...
	const currentSessionSelection = () => {
		try {
			...
			if (typeof current !== "string" || current.length === 0) return void 0;
			const projected = ctx.sessions.binding?.(current)?.session?.projections?.faceOf("modelSelection")?.getSnapshot();
			if (!isRecord(projected)) return void 0;
			return parseModelSelection(projected.next);
		} catch {
			return;
		}
	};
	const raced = (promise, label) => {
		return new Promise((resolve, reject) => {
			const timer = window.setTimeout(() => reject(new Error(label)), 1e4);
			promise.then((value) => {
	...
	const loadModels = async () => {
		try {
			if (!ctx.remote?.session?.modelCatalog) throw new Error("ctx.remote 不可用（remotes/gateway bundle 未加载；重启 dsh web 后请硬刷新页面）");
			const [catalogResult, defaultResult] = await Promise.allSettled([raced(ctx.remote.session.modelCatalog(), "模型目录请求超过 10 秒未响应"), ctx.connection.rpc.call(INLINE_CHAT_CHANNEL, "default-model", {}, modelController.signal)]);
			if (disposed) return;
			let defaultSelection;
			if (defaultResult.status === "fulfilled" && isRpcSuccess(defaultResult.value) && isRecord(defaultResult.value.value)) defaultSelection = parseModelSelection(defaultResult.value.value.selection);
			...
			if (catalogResult.status === "fulfilled") {
				...
				} else catalogError = "目录响应内容无效";
			} else catalogError = catalogResult.reason instanceof Error ? catalogResult.reason.message : String(catalogResult.reason);
			if (catalogLoaded) {
				...
				} else if (!selection || !findModel(modelGroups, selection)) selection = defaultSelection ?? firstSelection(modelGroups);
			} else if (!selection) selection = defaultSelection ?? firstSelection(modelGroups);
	...
	host.__dshInlineChatDispose = dispose;
	return { dispose };
}
...
function loadMessages() {
	try {
		...
		return value.filter((item) => isRecord(item) && (item.role === "user" || item.role === "assistant") && typeof item.text === "string");
	} catch {
		return [];
	}
}
...
function loadModelSelection() {
	try {
		...
		return raw ? parseModelSelection(JSON.parse(raw)) : void 0;
	} catch {
		return;
	}
}
...
function firstSelection(groups) {
...
function fitSelectWidth(el, maxWidth = 178) {
	if (typeof document === "undefined") return;
	const context = measureContext ?? (measureContext = document.createElement("canvas").getContext("2d") ?? void 0);
	if (!context) return;
	...
	const width = Math.min(Math.ceil(context.measureText(label).width) + 34, maxWidth);
	el.style.width = `${width}px`;
}
function renderMessage(message, open = false) {
	const errorClass = message.error ? " message-error" : "";
	const reasoning = message.reasoning ? `<details${open ? " open" : ""}><summary>思考过程<span class="chev">▸</span></summary><div class="reasoning">${escapeHtml(message.reasoning)}</div></details>` : "";
	if (message.role === "user") return `<article class="message user${errorClass}"><div class="message-text">${escapeHtml(message.text)}</div></article>`;
...
function isRpcSuccess(value) {
	return isRecord(value) && value.ok === true && "value" in value;
}
function isRpcFailure(value) {
