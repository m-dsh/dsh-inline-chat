...
function mount(ctx) {
	...
	const render = () => {
		...
		list.innerHTML = messages.length ? messages.map(renderMessage).join("") : emptyStateMarkup();
	...
	const loadModels = async () => {
		const [catalogResult, defaultResult] = await Promise.allSettled([ctx.connection.api.llm.models({}, modelController.signal), ctx.connection.rpc.call(INLINE_CHAT_CHANNEL, "default-model", {}, modelController.signal)]);
		if (disposed) return;
		...
		if (!selection || catalogLoaded && !findModel(modelGroups, selection)) selection = defaultSelection ?? firstSelection(modelGroups);
		if (selection) saveModelSelection(selection);
		modelLoading = false;
		modelNotice = catalogLoaded ? "" : selection ? "模型目录加载失败，已使用默认模型" : "没有可用模型，请先在 DSH 中配置";
		renderModelOptions();
	};
...
function renderMessage(message) {
	...
	const reasoning = message.reasoning ? `<details><summary>思考过程</summary><div class="reasoning">${escapeHtml(message.reasoning)}</div></details>` : "";
